...
class FundaSpider(scrapy.Spider):
    ...
    def __init__(self, place="amsterdam", range_max=300):
        self.start_urls = [
            "https://www.funda.nl/koop/%s/p%s/" % (place, page_number)
    ...
    def parse_dir_contents(self, response):
        ...
        bedrooms = response.xpath(
            "//span[contains(@title,'slaapkamer')]/following-sibling::span[1]/text()"
        ).extract()[0]
    ...
    def constructionYear(self, response):
        try:
            ...
            singleYear = response.xpath(
                "//dt[text()='Bouwjaar']/following-sibling::dd/span/text()"
            ).extract()
            # Some have a period
            period = response.xpath(
                "//dt[text()='Bouwperiode']/following-sibling::dd/span/text()"
            ).extract()
            if len(singleYear) > 0:
